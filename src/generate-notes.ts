...
function extractMessageText(obj: any): string | null {
  ...
  // Direct result from system or result message
  if (typeof obj.result === 'string' && (obj.type === 'result' || obj.role === 'system')) {
    return obj.result;
  }

  // Nested message object
  if (obj.message) {
    const nested = extractMessageText(obj.message);
    if (nested) {
      return nested;
    }
  }
...
export async function generateNotes(
  ...
  try {
    ...
    try {
      ...
      // Walk backwards to find the last valid message
      for (let i = parsedObjects.length - 1; i >= 0; i--) {
        const text = extractMessageText(parsedObjects[i]);
        if (text) {
          responseText = text;
          break;
        }
